...
export default function DealersPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const { data: res } = await api.get('/dealers', { params: { page, limit: 50, search } });
      setData(res.data);
  ...
  return (
    <div className="space-y-6">
      <div><h1 className="text-2xl font-bold text-gray-900">Dealers</h1><p className="text-gray-500 text-sm">{total.toLocaleString()} total</p></div>
      <div className="flex gap-3">
        <SearchBar value={search} onChange={(v) => { setSearch(v); setPage(1); }} placeholder="Search dealers..." />
